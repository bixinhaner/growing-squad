...
export function PetCreature({ species='bear', age='baby', action='idle', asleep=false, dress='', skill=3, dirt=0, className='' }) {
  ...
  const happy=['celebrate','hop','play','pat','cuddle','offer'].includes(action)
  ...
  return <svg viewBox="0 0 280 285" className={`pet-rig pet-rig--${action} pet-rig--${species} ${className}`} data-age={age} data-skill={skill} data-action={action} aria-hidden="true" focusable="false">
    ...
    <g className="rig-whole">
      ...
      <g className="rig-arm rig-arm--left"><ellipse cx="92" cy={bodyY-3} rx={cloud?21:17} ry={baby?26:33} transform={`rotate(22 92 ${bodyY-3})`} fill={cat?'#b1c4dc':fur} filter={`url(#${id}-felt)`} /><ellipse cx="83" cy={bodyY+13} rx="7" ry="9" fill={moonRabbit?'#efbac8':'#f6d7ba'} opacity={cloud?0:.65} /></g>
      <g className="rig-arm rig-arm--right"><ellipse cx="188" cy={bodyY-3} rx={cloud?21:17} ry={baby?26:33} transform={`rotate(-22 188 ${bodyY-3})`} fill={cat?'#b1c4dc':fur} filter={`url(#${id}-felt)`} /><ellipse cx="197" cy={bodyY+13} rx="7" ry="9" fill={moonRabbit?'#efbac8':'#f6d7ba'} opacity={cloud?0:.65} /></g>
      ...
      {(species==='bear' || dress==='scarf' || dress==='blue-scarf') && <g className="rig-bandana"><path d={`M108 ${bodyY-37}Q140 ${bodyY-24} 172 ${bodyY-37}L157 ${bodyY-8}Z`} fill="#728db7" /><path d={star} transform={`translate(149 ${bodyY-24}) scale(.32)`} fill="#ffe397" /></g>}
