...
export function PetPlay({ pet, session, others = [], onFinish, onDraft, quiet = false, timeLeft = Infinity }) {
  ...
  const progressRef=useRef({choices:[],work:null})
  ...
  useEffect(()=>{draftRef.current=onDraft},[onDraft])
  useEffect(()=>{if(!studioKind||!work?.title.trim()||finished.current)return;const timer=setTimeout(()=>draftRef.current?.(work),600);return()=>clearTimeout(timer)},[studioKind,work])
  ...
  return <section className="pet-play" aria-label={title}>
    ...
    {!achieved && !studioKind ? <div className="pet-play-controls">
      {session.game === 'ball' ? <><div className="pet-studio-tools" role="group" aria-label="选择滚球方向">{[['left','从左边'],['middle','从中间'],['right','从右边']].map(([v,n])=><button className="pet-button pet-button--soft" type="button" key={v} disabled={busy} aria-pressed={lane===v} onClick={()=>setLane(v)}>{n}</button>)}</div><button className="pet-button" type="button" disabled={busy} onClick={() => animate(lane==='middle'?'接住啦，轻轻推回来！':lane==='left'?'它先看看左边，再伸出爪子接住啦！':'转向右边，它抱住球，再推回来啦！', `一起滚球：${lane}`)}>{busy ? '球滚回来了…' : '把球轻轻推过去'}</button></> : null}
    ...
    </div> : achieved ? <button className="pet-button" type="button" disabled={elapsed < 3000} onClick={() => finish(true)}>{story ? '收下这段故事' : skill ? '收下这次小进步' : '这一轮玩好啦'}</button> : null}
    <footer className="pet-play-footer"><span>{elapsed >= limit-15000 ? '这一轮快到收尾时间啦。' : '随时可以停，不扣星光，也不会难过。'}</span><button className="pet-text-button" type="button" onClick={() => finish(false)}>现在回小屋</button></footer>
  </section>
}
