...
export function ChildShell() {
  ...
  return <div className={`child-app gs-child-app child-view-${viewName} theme-${profile.theme}`}>
    <header className="child-header gs-child-header">{coreView ? <Brand /> : <button className="calm-module-back" type="button" onClick={() => navigate('/world')}>‹ 返回小队世界</button>}<div className="gs-child-header__actions"><span className="calm-child-name">{profile.name}</span>
    ...
    </div></header><SaveIndicator /><main className="child-main gs-child-main"><Outlet /></main>
