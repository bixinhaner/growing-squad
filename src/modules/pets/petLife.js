...
export const CREATION = z.discriminatedUnion('kind', [
  z.object({ kind: z.literal('blocks'), title: z.string().trim().min(1).max(40), blocks: z.array(BLOCK).max(24) }),
  ...
  z.object({ kind: z.literal('drawing'), title: z.string().trim().min(1).max(40), strokes: z.array(z.object({ color: z.enum(['brown','green','blue','pink','yellow']), points: z.array(z.tuple([z.number().min(0).max(100), z.number().min(0).max(100)])).min(1).max(180) })).max(80) }),
])
export const GARDEN = z.object({ flower: z.enum(['daisy','tulip','star']), color: z.enum(['yellow','pink','blue']), place: z.enum(['left','middle','right']), note: z.string().trim().max(60) })
export const LIFE = z.object({
  preferences: z.record(z.string(), z.number().int().min(0).max(999)).default({}),
  preferenceDays: z.record(z.string(), z.array(z.string()).max(20)).default({}),
  ...
  layout: z.record(z.string(), POINT).default({}),
...
export function snapshotFor(pet, stage) {
  return { species:pet.species, name:pet.name, room:pet.room, stage, placed:{...pet.placed}, layout:{...(pet.life?.layout || {})} }
}
export const SNAPSHOT = z.object({ species:z.enum(['bear','rabbit','cloud','space-cat']),name:z.string().max(12),room:z.enum(['moon-room','forest','space']),stage:z.enum(['baby','young','companion','egg']),placed:z.record(z.string(),z.string().max(80)),layout:z.record(z.string(),POINT) })
