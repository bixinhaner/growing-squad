...
export function MovementParentPage() {
  ...
  return <section className="movement-parent">
    <header className="movement-parent__head"><div><span className="page-title__eyebrow">成长模块</span><h1>运动小队</h1><p>看见孩子愿意尝试什么，不比较运动量。</p></div><label className="rain-toggle"><span><Icon name="sparkle" />雨天模式：只推荐室内活动</span><input type="checkbox" checked={preferences.rainMode === true} onChange={(event) => updateRain(event.target.checked)} /></label></header>
    <div className="movement-insights"><article><span>参与过的运动</span><strong>{stats.completed.length}<small> 次</small></strong><p>{stats.observedCount ? `${stats.observedCount} 次有家长观察，其中 ${stats.autonomous} 次自行发起` : '仅凭点击开始，无法判断是否主动；尚无明确观察。'}</p></article><article><span>最喜欢</span><strong>{stats.favorite?.title || '还在发现'}</strong><p>只根据孩子说“还想玩”来判断</p></article><article><span>觉得有点难</span><strong>{stats.feedbackCounts.hard || 0}<small> 次</small></strong><p>推荐以每种活动的最新反馈为准</p></article></div>
