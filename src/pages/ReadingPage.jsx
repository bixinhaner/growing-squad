...
import './comfort.css'
import { JourneySteps } from '../ui/v2/Evolution.jsx'
...
export function ReadingShelfPage() {
  ...
  return <section className="reading-shelf" aria-labelledby="reading-shelf-title"><aside className="reading-treehouse-hero"><img src={appPath('assets/reading/story-treehouse-hero.webp')} alt="眠眠在故事树屋里等你" /><div><span className="eyebrow">阅读桥梁</span><h1 id="reading-shelf-title">故事树屋</h1><p>今天想和谁一起读？</p></div></aside><article className="reading-shelf__content"><JourneySteps current={0} />
    ...
    {books.length > 3 ? <label className="v2-book-search"><Icon name="search" /><input type="search" aria-label="找一本书" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="找找书名或作者" /></label> : null}
...
function ReadingPlayContent() {
  ...
  return <section className="reading-play"><img src={appPath('assets/reading/reading-companion.webp')} alt="眠眠抱着书，安静陪着你" /><article><JourneySteps current={1} /><span className="reading-mode-pill">{mode.title}</span><h1>故事在你手里</h1><p>{subtitle}，慢慢读就好</p><small>眠眠安静陪着你</small>{session.helpRequestedAt && !session.helpResolvedAt ? <div className="reading-helped" role="status">已记录你需要帮助，请叫家长来陪一下。</div> : null}<button className="reading-primary" type="button" onClick={() => dispatch({type:'COMPLETE_READING_SESSION',profileId:state.activeProfileId,sessionId})}>读完啦</button><button className="reading-help" type="button" onClick={() => dispatch({type:'REQUEST_READING_HELP',profileId:state.activeProfileId,sessionId})}>我需要帮助</button><button className="reading-return" type="button" onClick={() => navigate('/reading')}>先放回书架</button></article></section>
}
function ReadingReflection({session,book}) {
  ...
  return <section className="reading-reflection"><aside><BookCover book={book} /><span>故事叶收好啦</span><strong>{book.title}</strong></aside><article><JourneySteps current={2} /><h1>刚才读起来怎么样？</h1><p>可以说一点，也可以什么都不填。</p><div className="reading-difficulty">{DIFFICULTY_OPTIONS.map((item) => <button type="button" key={item.id} className={difficulty===item.id ? 'is-selected' : ''} aria-pressed={difficulty===item.id} onClick={() => setDifficulty(item.id)}><strong>{item.title}</strong><small>{item.copy}</small></button>)}</div><h2>想怎样告诉眠眠这个故事？</h2><div className="reading-reflection-options">{REFLECTION_OPTIONS.map((item) => <button type="button" key={item.id} className={reflection===item.id ? 'is-selected' : ''} aria-pressed={reflection===item.id} onClick={() => setReflection(item.id)}>{item.title}</button>)}</div><p className="reading-prompt">{prompt}</p>
