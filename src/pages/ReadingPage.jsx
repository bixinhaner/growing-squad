...
export function ReadingBookPage() {
  ...
  return <section className="reading-mode" aria-labelledby="reading-mode-title">
    ...
    <article><span className="eyebrow">选择陪伴方式</span><h2 id="reading-mode-title">这次想怎么读？</h2><div className="reading-mode-grid">{READING_MODES.map((mode, index) => <button type="button" key={mode.id} className={selected === mode.id ? 'is-selected' : ''} aria-pressed={selected === mode.id} onClick={() => setSelected(mode.id)}><img src={readingCover(['cloud-whisper', 'dream-train', 'pocket-ocean', 'talking-tree', 'fox-bridge', 'hedgehog-lantern', 'star-path', 'singing-tree'][index]).image} alt="" /><span><strong>{mode.title}</strong><small>{mode.subtitle}</small></span>{mode.recommended ? <em>今天试试</em> : null}</button>)}</div><div className="reading-mode-actions"><button className="reading-primary" type="button" onClick={start}>带我开始</button><button type="button" onClick={() => navigate('/reading')}>今天先不读</button></div></article>
  </section>
}
