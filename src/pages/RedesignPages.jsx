...
export function RedesignTonightPage() {
  ...
  return <section className={`gs-tonight-page${steps.length > 16 ? ' calm-tonight-overflow' : ''}`}>
    ...
    <article className="gs-tonight-tasks"><header><span><small>今晚任务</small><h1>今晚要做 {steps.length} 件事</h1></span><strong>{completeCount} / {steps.length} 已完成</strong><button type="button" onClick={() => setManageOpen(true)}><Icon name="menu" />调整今晚任务</button></header>
      <div className="gs-task-grid" style={{ '--count':steps.length }}>{steps.map((step,index) => <button type="button" key={step.id} className={(statuses[step.id] || 'todo') !== 'todo' ? 'is-done' : ''} onClick={() => toggle(step)} aria-pressed={statuses[step.id] === 'done'} aria-label={`${step.title}${statuses[step.id] === 'done' ? '，已完成，再点可撤销' : statuses[step.id] === 'skipped' ? '，今晚已跳过' : ''}`}><b>{index+1}</b><AssetArt id={step.icon} decorative /><strong>{step.title}</strong>{(statuses[step.id] || 'todo') !== 'todo' ? <Icon name="check" /> : null}</button>)}</div>
