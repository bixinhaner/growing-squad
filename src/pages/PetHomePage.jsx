...
const MEMORY_NAMES = { milestone: '成长时刻', play: '伙伴游戏', story: '想象故事', life: '来自生活', note: '我记的话', purchase: '小屋新物件' }
...
function PetHome() {
  ...
  async function endPlay(completed, choices) {
    if (!activeSession) return
    const next = await send('PET_END_PLAY', { sessionId: activeSession.id, completed, choices })
  ...
  return <section className="pet-home" aria-labelledby="pet-home-title" data-stage={growth.stage}>
    <header className="pet-heading"><div className="pet-title-group"><span className="pet-avatar"><PetActor species={pet.species} size="baby" /></span><div><span className="pet-eyebrow">{profile.name}的小伙伴</span><h1 id="pet-home-title">{pet.name}<button className="pet-text-button" type="button" aria-label="给小伙伴改名字" onClick={() => { setDraftName(pet.name); setRenameOpen(true) }}><Icon name="edit" size={17} /></button></h1><span className="pet-small-note">{growth.label} · {getSpecies(pet.species).name}</span></div></div>
    ...
    <div className="pet-home-status" role="status" aria-live="polite">{message || '基础照顾一直可用，星光让小屋多一点新玩法。'}</div>
    {activeSession ? <PetPlay key={activeSession.id} pet={pet} session={activeSession} others={Object.values(state.modules.pets.byProfile).filter((p) => p.id !== pet.id && p.hatchedAt)} onFinish={endPlay} quiet={quiet} /> : <>
      {tab === 'home' ? <>
        <div className={`pet-room pet-room--${pet.room} ${asleep ? 'is-quiet' : ''}`} aria-label={`${pet.name}的${PET_ROOMS.find((r) => r.id === pet.room).name}`}>
          ...
          <div className="pet-room-floor">
            ...
            {pet.hatchedAt ? <><button type="button" className="pet-room-object pet-room-object--food" onClick={() => care('feed')} disabled={busy} aria-label="给小伙伴准备食物"><PetProp kind="bowl" /><span>小饭碗</span></button><button type="button" className="pet-room-object pet-room-object--toy" onClick={() => setTab('play')} aria-label="打开玩具篮"><PetProp kind={pet.placed.toy ? getPetItem(pet.placed.toy).art : 'ball'} /><span>玩具们</span></button></> : null}
            {['lamp', 'decor'].map((slot) => pet.placed[slot] ? <button key={slot} type="button" className={`pet-room-decoration pet-room-decoration--${slot} pet-decoration-position--${pet.positions[slot] || 'right'} ${slot === 'lamp' && lightOn ? 'is-lit' : ''}`} aria-pressed={slot === 'lamp' ? lightOn : undefined} aria-label={`看看${getPetItem(pet.placed[slot]).name}`} onClick={() => { if (slot === 'lamp') { setLightOn((on) => !on); setMessage(lightOn ? '小灯休息了。再点一下就会亮。' : '一颗温暖的小星星，亮起来了。') } else { animate('celebrate'); setMessage(`这是你给小屋挑的${getPetItem(pet.placed[slot]).name}。`) } }}><PetProp kind={getPetItem(pet.placed[slot]).art} /></button> : null)}
          </div>
    ...
    {hatching ? <Modal title="我们的第一次见面" onClose={() => setHatching(false)} className="pet-modal pet-hatch-modal"><span className="pet-eyebrow">可以重看，不用重新孵蛋</span><h2>你好呀，{pet.name}</h2><div className="pet-hatch-scene" data-reduced={getAccessibility(state).reduceMotion}><PetActor species={pet.species} pose="wave" size="baby" /><div className="pet-hatch-shell"><EggArt species={pet.species} crack={2} /></div><span className="pet-hatch-sparkles" aria-hidden="true">✦ ✧ ✦</span></div><p>{getSpecies(pet.species).name}从你选的蛋里，来到了这个小家。</p><button className="pet-button" type="button" onClick={() => setHatching(false)}>抱抱我的小伙伴</button></Modal> : null}
