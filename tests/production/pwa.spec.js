...
test('pet can be adopted and hatched on its first offline visit with all four companion atlases cached', async ({ context, page }) => {
  await setupFamily(page,{at:'2026-09-20T16:00:00+08:00'})
  await page.evaluate(()=>navigator.serviceWorker.ready)
  await expect.poll(()=>page.evaluate(()=>Boolean(navigator.serviceWorker.controller))).toBe(true)
  await context.setOffline(true)
  await page.goto('/bedtime/pet',{waitUntil:'domcontentloaded'})
  await page.getByRole('button',{name:/云朵蛋/}).click()
  await page.getByRole('button',{name:'把这颗蛋带回家'}).click()
  await expect(page.getByRole('heading',{name:'绵绵',exact:true})).toBeVisible()
