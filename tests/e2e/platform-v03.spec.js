...
test('parent timeline previews the child card and support changes remain parent-confirmed', async ({ page }) => {
  ...
  const packBagSupport = page.getByRole('group', { name: '整理书包的支持方式' })
  await packBagSupport.getByRole('button', { name: '自己开始' }).click()
  await expect(packBagSupport.getByRole('button', { name: '自己开始' })).toHaveClass(/is-active/)
  await expect(page.locator('.support-suggestion')).toHaveCount(0)
  await expect(page.getByText(/随时增加陪伴，不代表退步/)).toBeVisible()
